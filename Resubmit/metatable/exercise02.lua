...
local patientMeta = {
    -- Implement the __index metamethod to retrieve patient information securely
    __index = function(tbl, key)
      if key == "diagnosis" then
        -- Perform authentication check here before allowing access
        print("Authentication required to access diagnosis.")
        return nil -- Return nil to deny access
      else
        -- Allow access to other fields without authentication
        return tbl[key]
      end
    end,
  
    -- Implement the __newindex metamethod to restrict unauthorized modifications
    __newindex = function(tbl, key, value)
      if key == "diagnosis" then
        -- Perform authentication check here before allowing modification
        print("Authentication required to modify diagnosis.")
      else
        -- Allow modification of other fields without authentication
        rawset(tbl, key, value)
      end
    end
  }
  
  -- Define a table for patient records with the patientMeta metatable
